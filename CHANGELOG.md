...
- **`git-sfs llms-txt`** — new subcommand that prints a fully self-contained LLM reference document embedded in the binary. Covers the core model, all commands, configuration, env vars, workflows, and safety notes. Works entirely offline; no network access required.
- **`llms.txt` at repo root** — LLM-friendly reference for agents browsing the GitHub repo before installing git-sfs. Generated by `scripts/gen_llms_txt.py` from the existing docs; regenerate with `just gen-llms-txt` when docs change.
- **`skills/git-sfs/` Agent Skill** — a Claude Code agent skill (`SKILL.md`) that users can copy to `~/.claude/skills/git-sfs/` (global) or `.claude/skills/git-sfs/` (project) to give their coding agent context about git-sfs. When triggered, the skill runs `git-sfs llms-txt` for the full embedded reference.
