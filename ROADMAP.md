...
- `status` — show tracked file sizes and local cache / remote presence at a glance, without downloading; optional `--remote NAME` metadata check and `--json` output
- `remotes` — list the remotes configured in `.git-sfs/config.toml` (name, backend, path, config), with `--json`
